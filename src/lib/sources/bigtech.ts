...
/**
 * Microsoft runs its own search service behind jobs.careers.microsoft.com.
 * The list endpoint carries locations and posting dates; descriptions need a
 * second call per job, which is worth it only for the ones that classify.
 */
...
interface MicrosoftSearch {
  operationResult?: {
    result?: { jobs?: MicrosoftJob[]; totalJobs?: number };
  };
}

const MS_PAGE = 20;
const MS_MAX_PAGES = 20;
const MS_MAX_DETAILS = 200;
const MS_QUERIES = ['internship', 'intern', 'student'];

const MS_INTERNISH =
  /\bintern(?:ship|ships|s)?\b|\bco-?ops?\b|\bapprentice|\bstudent\b|\bplacement\b|\buniversity\b|\bgraduate\s+program\b/i;

export async function fetchMicrosoft(): Promise<RawListing[]> {
  const found = new Map<string, MicrosoftJob>();
  const opts = { timeoutMs: 40_000, tolerate: [400, 403, 404, 410, 422] };

  for (const query of MS_QUERIES) {
    for (let pageIndex = 1; pageIndex <= MS_MAX_PAGES; pageIndex++) {
      const url =
        `https://gcsservices.careers.microsoft.com/search/api/v1/search` +
        `?q=${encodeURIComponent(query)}&l=en_us&pg=${pageIndex}&pgSz=${MS_PAGE}&o=Relevance&flt=true`;
      const data = await getJson<MicrosoftSearch>(url, opts);
      const jobs = data?.operationResult?.result?.jobs ?? [];
      for (const job of jobs) if (job.jobId) found.set(job.jobId, job);
      if (jobs.length < MS_PAGE) break;
    }
  }

  const candidates = [...found.values()].filter((job) => MS_INTERNISH.test(job.title ?? ''));

  for (const job of candidates.slice(0, MS_MAX_DETAILS)) {
    try {
      const detail = await getJson<{ operationResult?: { result?: MicrosoftJob } }>(
        `https://gcsservices.careers.microsoft.com/search/api/v1/job/${encodeURIComponent(job.jobId!)}?lang=en_us`,
        { ...opts, retries: 1 },
      );
      const full = detail?.operationResult?.result;
      if (full) {
        job.properties = { ...job.properties, ...full.properties };
        // The detail payload puts the body at the top level on some responses.
        const loose = full as MicrosoftJob & Record<string, unknown>;
        for (const key of ['description', 'responsibilities', 'qualifications'] as const) {
          const value = loose[key];
          if (typeof value === 'string' && value) job.properties![key] = value;
        }
      }
    } catch {
      // Title-only still classifies; the description just adds precision.
    }
  }
  ...
  return candidates
    .map((job): RawListing => {
      ...
      const description = [p.description, p.responsibilities, p.qualifications]
        .filter(Boolean)
        .join('\n\n');
      return {
        source: 'microsoft',
        ...
        sourceId: job.jobId!,
        company: 'Microsoft',
        companyUrl: 'https://jobs.careers.microsoft.com',
        title: job.title ?? '',
        applyUrl: `https://jobs.careers.microsoft.com/global/en/job/${job.jobId}`,
        description: description || null,
        locations: [p.primaryLocation, ...(p.locations ?? [])].filter(Boolean) as string[],
        remoteFlag: p.workSiteFlexibility ? /remote|up to 100%/i.test(p.workSiteFlexibility) : null,
        terms: [],
        datePosted: secs(job.postingDate),
        dateUpdated: secs(job.postingDate),
        categoryHint: [p.profession, p.discipline].filter(Boolean).join(', '),
        activeFlag: true,
        tags: [p.employmentType, p.roleType].filter(Boolean).map((t) => `employment:${t}`),
      };
    })
